    ...
    <taon-admin-mode-configuration>
      @if (itemsLoaded()) {
        ...
        <footer
          ...
          Copyright <strong>main</strong> {{ year }}
        </footer>
      }
    </taon-admin-mode-configuration>
  `,
...
export class MainApp implements OnInit {
  ...
  navItems =
    MainClientRoutes.length <= 1
      ? []
      : MainClientRoutes.filter(r => r.path !== undefined).map(r => ({
          path: r.path === '' ? '/' : `/${r.path}`,
...
export const MainClientRoutes: Routes = [
  {
    path: '',
    pathMatch: 'full',
    redirectTo: () => {
      if (MainClientRoutes.length === 1) {
        return '';
      }
      return MainClientRoutes.find(r => r.path !== '')!.path!;
    },
  },
  // PUT ALL ROUTES HERE
...
export const MainServerConfig: ApplicationConfig = {
  providers: [provideServerRendering(withRoutes(MainServerRoutes))],
};

export const MainConfig = mergeApplicationConfig(
  MainAppConfig,
  MainServerConfig,
);
//#endregion
...
//#region  main start function
export const MainStartFunction = async (
...
export default MainStartFunction;
//#endregion
